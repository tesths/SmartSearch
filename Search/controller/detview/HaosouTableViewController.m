...
@interface HaosouTableViewController ()

@end

@implementation HaosouTableViewController
...
- (void)viewDidLoad {
    ...
    [self.tableView addFooterWithTarget:self action:@selector(loadHaosou)];
...
- (void)StartSearch
{
    ...
    [[SearchApi searchhaosou:self.searchstr] subscribeNext:^(id value) {
...
- (void)loadHaosou
{
    ...
    [[SearchApi searchnexturl:self.searchmodel.NextPage andParsename:@"HaosouData"] subscribeNext:^(id value) {
        NSString *className = [NSString stringWithUTF8String:object_getClassName(value)];
        if ([className isEqualToString:@"RACDynamicSignal"]) {
            [value subscribeNext:^(id x) {
                self.searchmodel = x;
