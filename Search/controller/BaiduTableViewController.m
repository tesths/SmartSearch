...
- (void)viewDidLoad {
    ...
    [self.tableView addFooterWithTarget:self action:@selector(loadBaidu)];
...
- (void)StartSearch
{
    ...
    [[SearchApi searchbaidu:self.searchstr] subscribeNext:^(id value) {
...
- (void)loadBaidu
{
    ...
    [[SearchApi searchnexturl:self.searchmodel.NextPage andParsename:@"BaiduData"] subscribeNext:^(id value) {
        NSString *className = [NSString stringWithUTF8String:object_getClassName(value)];
        if ([className isEqualToString:@"RACDynamicSignal"]) {
            [value subscribeNext:^(id x) {
                self.searchmodel = x;
